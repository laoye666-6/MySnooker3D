...
public static class G
{
    ...
    // ------------------------------------------------------------------
    // 二b、加塞（杆法）物理参数 —— v0.36 新增，v0.37 按真实台呢参数重定标
    //
    // 真实台球的自旋无法用 PhysX 表达（刚体+库仑摩擦只会把自旋抹平），
    // 因此白球的自旋由 BallController 里的"滑移摩擦"模型自己算：
    //   接触点滑移速度 u = v + ω × r_c，摩擦力沿 -û 同时改线速度与角速度，
    //   自旋与滚动自然收敛 —— 跟杆/低杆是模型涌现的结果。
    //
    // v0.37 定标依据（实测文献值：Dr Dave Billiards / Colorado State 台球物理）：
    //   球-台呢滑动摩擦系数 μ_slide ≈ 0.2（范围 0.15~0.4）→ a = μ·g ≈ 1.962 m/s²
    //   球-台呢滚动阻力系数 0.005~0.015（RollDecel=0.11 ≈ μ_roll 0.011，在范围内）
    //   球重 0.17kg：a=μ·g 与质量无关（质量约掉）——"结合球重"的正确姿势是
    //   加速度只取 μ·g、严禁由自旋凭空造加速度；质量只影响动量与对库冲量。
    //
    // v0.37 修正的两个超现实参数（杆头击点物理极限）：
    //   真实杆头击点最多偏到 h≈0.4~0.5r（再远就滑杆 miscue）。由冲量公式
    //   ω·r/v = (5/2)·(h/r)：高杆极限 +1.25×(v/r)，低杆极限 -1.0×(v/r)（纯倒旋）。
    //   旧版 SpinTopK=1.5 给到 2.5 倍滚动 → 打滑期近 1 秒、白球被加速约 2m/s
    //   （"大力高杆加速过快"根因）；旧版 SpinLowK=1.6 只有 -0.6 倍倒旋（"低杆不明显"）。
    //
    // 限速约束：白球任何时刻速度 ≤ 出杆初速（BallController.shotSpeed0 钳制）。
    // ------------------------------------------------------------------

    /// 高杆强度：ω = (1 + spinV·SpinTopK)×(v/r)。
    /// 1.0（ω=2×滚动）超出真实杆头极限，但 v0.37 限速钳制保证白球速度【任何时刻不超
    /// 出杆初速】——多余自旋只会转化为"撞球后的跟进"，绝不会变成途中加速。
    /// 取 1.0 是因为真实上限(1.25×滚动)的多余自旋在 0.75m 行程内就被滑移摩擦
    /// 自然耗尽（a=μ·g 物理如此，离机实测 0.25/0.5 均无感），手感上高杆等于消失；
    /// 在限速保护下取 1.0 才能保有可用跟进。这是"观感优先、限速兜底"的取舍。
    public const float SpinTopK = 1.0f;

    /// 低杆强度：ω = (1 - spinV·SpinLowK)×(v/r)。2.0 → spinV=-1 恰为 -1.0×(v/r)
    /// 纯倒旋（真实极限），撞球后明显拉回。副产物：spinV≈-0.5 时 ω≈0 = 真实
    /// 斯诺克的【定杆 stun】（打中心偏下），HUD 文案按此分档。
    public const float SpinLowK = 2.0f;

    /// 左右塞强度：ω 竖直分量 = spinH × SpinSideK × (v / BallR)（见 BallController 符号说明）。
    public const float SpinSideK = 0.7f;

    /// 打滑滑动摩擦减速度（m/s²）= μ·g = 0.2 × 9.81（台呢实测典型值）。
    public const float SlipDecel = 1.962f;
