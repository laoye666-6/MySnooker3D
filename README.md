...
> 截图取自 v0.40 实机（MuMu）。**加塞击球点**：拖动圆盘内的小圆点选高杆/低杆/左右塞，
...
- **加塞 / 杆法**：高杆（跟杆）、低杆（缩杆）、定杆、左右塞（吃库改角）。
  按真实台呢参数定标（滑动摩擦 μ≈0.2），且**白球速度任何时刻不超过出杆初速**。
...
| 力度 | 右下角滑条 |
| 击球 | 右下角"击球"按钮 |
| **加塞 / 杆法** | 右下角**「击球点」盘**：拖动盘内小圆点 —— 圆心=中杆、向上=高杆（跟杆）、向下=低杆（缩杆）、偏下约一半=**定杆**（撞球即停）、左右=左右塞（吃库改角）。白球速度任何时刻不超过出杆初速 |
| **设置** | 主菜单或 HUD 左下「设置」：帧率上限 / 渲染分辨率 / 画面阴影 / **物理步长（0.5 / 1 / 2ms）**，改动立即生效并持久化 |
...
- **加塞（杆法）按真实台呢参数重定标**。旧版两个参数都不符合现实：
  - 大力高杆**加速过快**：原先给了 2.5 倍滚动自旋，远超真实球杆的击点极限。
    按实测文献（球-台呢滑动摩擦系数 **μ≈0.2**、滚动阻力 0.005~0.015），
    并由杆头击点极限 `ω·r/v = (5/2)·(h/r)` 推出：高杆上限约 1.25 倍、低杆为 -1.0 倍（纯倒旋）。
  - **低杆效果不明显**：原先只有 0.6 倍倒旋。
  - **新增硬限速**：白球任何时刻速度**不得超过出杆初速**，大力高杆也不会越打越快。
  - 副产物：击球点拖到偏下约一半时 ω≈0，即真实的**定杆（stun）**，撞球即停。
- **物理步长可在设置里调**：新增 **0.5ms / 1ms / 2ms** 三档（默认 2ms），
  改动立即生效，**重启后沿用**（PlayerPrefs 持久化）。
- **界面重做为原神风**：所有按钮做成「金色细描边 + 深藏青底 + 四角菱形饰钉」，
  主操作按钮带宝石菱；力度滑条改金色轨道 + 菱形手柄；设置面板加行间金线；
  菜单标题加双翼金线与中央宝石；HUD 顶栏加金线；加塞圆盘外圈加金菱刻度。
- **修复**：设置面板打开时 HUD 文字穿透面板标题；菜单两个按钮边缘贴合。
- 新增编辑器内**自动截图驱动** `Editor/ShotTest.cs`：一条时间线跑完菜单/对局/设置/加塞/
  击球全过程并输出 9 张关键帧，让界面迭代不必每轮都装模拟器。
